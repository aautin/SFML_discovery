...
Animation::Animation(size_t frameSize, size_t frameTime, size_t framesNbs,
	...
	: _frameIndex(0), _frameSize(frameSize), _frameTime(frameTime), _framesNbs(framesNbs), _obj(texture), _startTimestamp(startTimestamp)
{
	setFrame();
}
...
{
	_obj.setTextureRect(sf::IntRect(sf::Vector2i(_frameIndex * _frameSize, 0), sf::Vector2i(_frameSize, _frameSize)));
...
void Animation::reset(unsigned long startTimestamp)
{
	if (startTimestamp)
